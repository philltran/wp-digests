...
(Trac + wordpress-develop), the block editor (Gutenberg), WooCommerce, and the Make
WordPress dev blogs — filtered by impact and community interest.

Each notable change is one greppable Markdown file under [`issues/`](issues/), indexed
into per-track RSS feeds under [`feeds/`](feeds/). Updated weekly.
...
- **Sources.** Summaries are derived from public, openly-licensed sources and each entry
  ...
  commit, or a Make WordPress post). Upstream content remains under its own license
